...
int AplphaBetaPruning(int depth, int nodeIndex, bool maximizingPlayer, vector<int> values, int alpha, int beta){
...
void funcion(int depth, int nodeIndex, bool isMax, vector<int> scores){
	result = max(Minimax(depth,nodeIndex,isMax,scores), AplphaBetaPruning(depth,nodeIndex,isMax,scores,MIN,MAX));
}
...
int main(){
	...
	char message[1000000], server_reply[1000000], reply[1000000];
	...
	while(1){
		...
		if(recv(sock, server_reply, 1000000,0)){
			...
			n = scores.size();
	    h = log2(n);
			std::cout << Minimax(0,0,true,scores) << '\n';
